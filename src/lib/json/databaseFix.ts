// Assuming your original JSON array is assigned to a variable called 'plantData'
...
const mergePlants = ( data: PlantData[] ) => {
  const mergedObj = data.reduce(
    (
      ...
      if ( !acc[ key ] ) {
      // First time seeing this plant: create a shallow copy
      // We also ensure arrays are safely copied so we don't mutate the original data
        acc[ key ] = {
          ...current,
      ...
      // Plant exists: merge the new data into the existing object
        ...
        // Merge the taxon object safely
        if ( current.taxon ) {
          existing.taxon = {
            family : existing.taxon?.family || current.taxon.family,
            genus  : existing.taxon?.genus || current.taxon.genus,
            species: existing.taxon?.species || current.taxon.species,
          };
        }
      }
...
const finalCleanArray = mergePlants( plantData );
console.log( JSON.stringify( finalCleanArray ) );
console.log( finalCleanArray );

// 2. Write the processed data to a new JSON file
try {
